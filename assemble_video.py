...
def mux_audio(video_path: Path, narration_path: Path, music_path: Path | None, output_path: Path):
    if music_path and music_path.exists():
        # صوت الراوي كامل + موسيقى خلفية منخفضة جدًا تحته
        filter_complex = (
...
def main():
    ...
    mux_audio(concatenated_path, NARRATION_AUDIO, music_path, FINAL_OUTPUT)

    print(f"✅ الفيديو النهائي جاهز: {FINAL_OUTPUT}")
    print(f"   المدة الإجمالية: ~{narration_duration:.1f} ثانية")
